 ...
 * @generated
 */
public class LimitBoxItemSemanticEditPolicy extends UcdiagramBaseItemSemanticEditPolicy {

	/**
	* @generated
	*/
	public LimitBoxItemSemanticEditPolicy() {
	...
	*/
	private void addDestroyChildNodesCommand(ICompositeCommand cmd) {
		...
		for (Iterator<?> nit = view.getChildren().iterator(); nit.hasNext();) {
			...
			switch (UcdiagramVisualIDRegistry.getVisualID(node)) {
			case LimitBoxLimitBoxUseCasesCompartmentEditPart.VISUAL_ID:
				for (Iterator<?> cit = node.getChildren().iterator(); cit.hasNext();) {
					Node cnode = (Node) cit.next();
					switch (UcdiagramVisualIDRegistry.getVisualID(cnode)) {
					case UseCaseEditPart.VISUAL_ID:
						for (Iterator<?> it = cnode.getTargetEdges().iterator(); it.hasNext();) {
							Edge incomingLink = (Edge) it.next();
							if (UcdiagramVisualIDRegistry
									.getVisualID(incomingLink) == ComRelationshipEditPart.VISUAL_ID) {
							...
							if (UcdiagramVisualIDRegistry
									.getVisualID(incomingLink) == IncludeRelationshipEditPart.VISUAL_ID) {
								DestroyElementRequest r = new DestroyElementRequest(incomingLink.getElement(), false);
								cmd.add(new DestroyElementCommand(r));
								cmd.add(new DeleteCommand(getEditingDomain(), incomingLink));
								continue;
							...
							if (UcdiagramVisualIDRegistry
									.getVisualID(incomingLink) == ExtendsRelationshipEditPart.VISUAL_ID) {
								DestroyElementRequest r = new DestroyElementRequest(incomingLink.getElement(), false);
								cmd.add(new DestroyElementCommand(r));
								cmd.add(new DeleteCommand(getEditingDomain(), incomingLink));
								continue;
						...
						for (Iterator<?> it = cnode.getSourceEdges().iterator(); it.hasNext();) {
							...
							if (UcdiagramVisualIDRegistry
									.getVisualID(outgoingLink) == IncludeRelationshipEditPart.VISUAL_ID) {
							...
							if (UcdiagramVisualIDRegistry
									.getVisualID(outgoingLink) == ExtendsRelationshipEditPart.VISUAL_ID) {
								DestroyElementRequest r = new DestroyElementRequest(outgoingLink.getElement(), false);
								cmd.add(new DestroyElementCommand(r));
								cmd.add(new DeleteCommand(getEditingDomain(), outgoingLink));
								continue;
